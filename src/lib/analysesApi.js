...
/** Published analyses, most recent first - includes the verdict so the
 * ledger can show it without fetching each full analysis payload. */
export async function listPublicAnalyses(limit = 100) {
  assertConfigured()
  const { data, error } = await supabase
    .from('analyses')
    .select('id, ticker, company_name, created_at, results_json->valuationSummary->verdict')
    .eq('is_public', true)
